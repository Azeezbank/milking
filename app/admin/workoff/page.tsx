...
//             `${backendUrl}/api/v1/admin/off?month=${month}&year=${year}`,
...
//         await axios.put(
//             `${backendUrl}/api/v1/admin/off/${id}`,
//             { newDate },
//             { withCredentials: true }
//         );
//         fetchOverview();
...
export default function AdminWorkOffPage() {
  ...
  const fetchLimit = async () => {
    try {
      const res = await axios.get(`${backendUrl}/api/v1/off/limit`, {
        withCredentials: true,
      });
              ...
              {records.map((r) => (
                <tr key={r.id} className="border-t hover:bg-slate-50 transition">
                  ...
                  <td>
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${
                        r.used ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"
                      }`}
                    >
                      {r.used ? "Used" : "Available"}
                    </span>
